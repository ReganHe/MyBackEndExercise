    ...
    public class weixinOAuth : OAuth2Base
    {
        ...
        internal override string OAuthUrl
        {
            get
            {
                ...
                return "https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&scope=snsapi_login&redirect_uri={1}&state={2}";
            }
        }
                        ...
                        if (!string.IsNullOrEmpty(token))
                        {
                            ...
                            if (!string.IsNullOrEmpty(openID))
                            {
                                ...
                                if (!string.IsNullOrEmpty(result)) //返回：callback( {"client_id":"YOUR_APPID","openid":"YOUR_OPENID"} ); 
                                {
                                    ...
                                    headUrl = Tool.GetJosnValue(result, "headimgurl");//获取头像
                                    return true;
                                }
                            }
                        }
