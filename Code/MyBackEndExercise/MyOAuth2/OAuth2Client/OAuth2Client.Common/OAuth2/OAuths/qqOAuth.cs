...
namespace OAuth2Client.Common.OAuth2.OAuths
{
    public class qqOAuth : OAuth2Base
    {
        ...
        {
            return string.Format(OAuthUrl, AppKey, System.Web.HttpUtility.UrlEncode(CallbackUrl), "qq");
        }
        public override bool Authorize()
        {
            if (!string.IsNullOrEmpty(code))
            {
                string result = GetToken("GET", "qq");//一次性返回数据，QQ仅返回Token，还需要一次请求获取OpenID。//access_token=A5E175586196173434374BD3DBBAA5E8A3&expires_in=7776000
                //分解result;
                        ...
                        if (!string.IsNullOrEmpty(token))
                        {
                            ...
                            if (!string.IsNullOrEmpty(openID))
                            {
                                ...
                                if (!string.IsNullOrEmpty(result)) //返回：callback( {"client_id":"YOUR_APPID","openid":"YOUR_OPENID"} ); 
                                {
                                    ...
                                    headUrl = Tool.GetJosnValue(result, "figureurl_2");
                                    //headUrl = Tool.GetJosnValue(result, "figureurl");//获取头像
                                    return true;
                                }
                            }
                        }
